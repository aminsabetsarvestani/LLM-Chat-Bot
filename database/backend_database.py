...
async def add_conversation( input: dict):
...
async def update_conversation(input):
    user = db.query(User).filter(User.username == input["username"]).first()
    if not user:
        db.close()
...
async def retrieve_conversation(input):
    user = db.query(User).filter(User.username == input["username"]).first()
    if not user:
        db.close()
...
async def add_collection(input):
    user = db.query(User).filter(User.username == input["username"]).first()
...
@app.post("/check_collection_exists")
async def check_collection_exists(input):
    user = db.query(User).filter(User.username == input["username"]).first()
    if not user:
        return {"error": "User not found"}
    if input["username"] and input["username"][0].isalpha():
...
async def get_collections(input):
    user = db.query(User).filter(User.username == input["username"]).first()
    if not user:
        return {"error": "User not found"}
...
async def delete_collection(input):
    user = db.query(User).filter(User.username == input["username"]).first()
    if not user:
        return {"error": "User not found"}
    if input["username"] and input["username"][0].isalpha():
...
if __name__ == "__main__":
    uvicorn.run("backend_database:app", host="0.0.0.0", port=API_port, reload=True)
